...
class LatestOfferCollCell: UICollectionViewCell {
    ...
    func UpdateView(offer:Offer) {
        ...
        SalonRateStars.value = CGFloat(truncating: NumberFormatter().number(from: offer.salon_rate ?? "0.0") ?? 0)
        offerDescription.text = offer.offer_description ?? ""

         offerTimer.text = "\(offer.days ?? "0")D: \(offer.hours ?? "0")H: \(offer.minutes ?? "0")M  \(offer.seconds ?? "0")sec"
       ...
       // offerPrice.attributedText = amountText
        offerPrice.text = "\(offer.new_price ?? "0") \(offer.currency ?? "")"
        runTimer()
    }
    
    func UpdateView(offer:OfferSearchData) {
        SetImage(image: offerImage, link: offer.image ?? "")
        offerName.text = offer.offer_name ?? ""
        SalonName.text = ""
        //SalonRateStars.value = CGFloat(truncating: NumberFormatter().number(from: offer.salon_rate ?? "0.0") ?? 0)
