            ...
            if tmp == nil {
                ...
                do {
                    ...
                    if (self.current_page == 1){
                        ...
                        self.salonOffers.data?.offers?.append(contentsOf: (decoded_data.data?.offers)!)
                    }
...
//extension CenterOffersVC: UITableViewDelegate , UITableViewDataSource {
//
//    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
//
//        if requested && salonOffers.data?.offers?.count ?? 0 == 0 {
//            NoOffersView.isHidden = false
//        }else {
//            NoOffersView.isHidden = true
//        }
//
//        return salonOffers.data?.offers?.count ?? 0
//    }
//
...
//            var currency = ""
//
//            salonOffers.data?.offers?[indexPath.row].branches?.forEach({ (branch) in
//                if branch.main_branch == "1" {
//                    currency = branch.currency?.currency_name ?? ""
//                }
//            })
//
//            cell.UpdateView(offer: salonOffers.data?.offers?[indexPath.row] ?? SalonOffer() , currency: currency)
//            return cell
//        }
//
...
//            vc.OfferID = "\(salonOffers.data?.offers?[indexPath.row].id ?? Int())"
...
extension CenterOffersVC : UICollectionViewDelegate, UICollectionViewDataSource , UICollectionViewDelegateFlowLayout  {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        let pager = (salonOffers.data?.offers?.count ?? 0 >= 1) ? (has_more_pages ? 1 : 0): 0
        print("pager items num ==> \(pager)")

        return (salonOffers.data?.offers?.count ?? 0) + pager
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if (indexPath.row >= (salonOffers.data?.offers?.count ?? 0)) {
