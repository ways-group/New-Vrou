...
class MyWorldTableCell: UITableViewCell{
    ...
    func UpdateView(offer:Offer, index:Int) {
        ...
        //
        PriceLbl.attributedText = amountText
    ...
    func UpdateView_notifications(offer:Offer) {
    ...
    func UpdateView(offer:MyFavourite) {
    ...
    func UpdateView(product:MyFavourite) {
        
        SetImage(image: SalonImage, link: product.salon_logo ?? "")
        SalonName.text = product.salon_name ?? ""
        SalonCategory.text = product.category_name ?? ""
        SetImage(image: OfferImage, link: product.main_image ?? "")
        OfferName.text = product.product_name ?? ""
        OfferDescription.text = product.product_description ?? ""
        
        
        let currency = " \(product.currency ?? "")"
        let finalString = (product.price ?? "0") + currency
        let amountText = NSMutableAttributedString.init(string: finalString)
        amountText.setAttributes([NSAttributedString.Key.font: UIFont.systemFont(ofSize: 12, weight: .semibold),NSAttributedString.Key.foregroundColor: #colorLiteral(red: 0.6833723187, green: 0.1359050274, blue: 0.4578525424, alpha: 1)], range: NSMakeRange(finalString.count-currency.count,currency.count))
        // set the attributed string to the UILabel object
        //
...
extension MyWorldTableCell : UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout  {
